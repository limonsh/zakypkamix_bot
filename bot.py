...
def default_data():
    return {
        ...
        "template": {
            ...
            "japan": ["Sous", "Ris", "Vasabi"],
            "bazar": ["Pomidory", "Zelen", "Ogurcy"],
            "endis": ["Muka", "Sahar", "Maslo2"],
        },
        "zayavka": {}, "zakupka_status": {}, "zakazano": [],
        "carry": {}, "comments": {},
        "last_date": str(date.today()), "pinned_msg_id": None,
    }
def load():
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("z:edit:"):
        ...
        await q.message.reply_text("Kommentarij k " + name + ":\n\nNapishi tekst ili /skip")
...
async def cfg_additem_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not name:
        await update.message.reply_text("Pustoe nazvanie, eshche raz:")
        return ST_ADD_ITEM_NAME
...
async def cfg_addsec_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Emoji dlya razdela:\n\nNaprimer: 🥩 🐟\n\n/skip - bez emoji")
